...
@Component({
  selector: 'admin-debit-from-account-page',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './debit-from-account-page.component.html',
})
export class DebitFromAccountPageComponent {
  ...
  onDebit(): void {
    ...
    this.financialService.debit(payload).subscribe({
      next: () => {
      ...
      error: (err) => {
        this.errorMessage = err?.error?.message || 'Debit failed. Please try again.';
